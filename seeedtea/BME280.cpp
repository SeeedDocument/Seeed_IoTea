...
float BME280::getTemperature(void)  //error code 0
{
  int32_t var1, var2;
...
uint32_t BME280::getPressure(void)  //error code 0
{
  int64_t var1, var2, p;

  // Call getTemperature to get t_fine
  if (getTemperature() == 0){  //此处更改。接受到上级错误后返回0并退出
    return 0;
  }
  else{
    ...
    if (var1 == 0)
    {
      return 0; // avoid exception caused by division by zero  此处更改为0
    }
    p = 1048576-adc_P;
...
uint32_t BME280::getHumidity(void)  //error code 0
{
...
uint8_t BME280::BME280Read8(uint8_t reg)
{
  ...
  for (int try_time = 0;try_time<2;try_time++){
    if(Wire.available())
      return Wire.read();
  }
  return 0;
